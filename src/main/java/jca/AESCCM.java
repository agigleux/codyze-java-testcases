...
public class AESCCM extends BCBase { 
	...
	public static void main(String[] args) throws NoSuchAlgorithmException, NoSuchPaddingException, NoSuchProviderException, InvalidKeyException, InvalidAlgorithmParameterException {
		Cipher c = Cipher.getInstance("AES/CCM/NoPadding", "BC"); // Compliant: BC as Provider, AES+CCM is OK
		// is it OK to have "NoPadding" for AES/CCM?
		 
		System.out.println(c.getAlgorithm());
		
		KeyGenerator kg = KeyGenerator.getInstance("AES", "BC"); // Compliant: BC as Provider
		kg.init(128);
		...
		SecureRandom sr = SecureRandom.getInstance("Default", "BC"); // Compliant: BC as Provider
		sr.nextBytes(iv);
